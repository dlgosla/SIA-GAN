...
class Generator_Transformer(nn.Module):
    def __init__(self, bs, ntoken=128, ninp=50, nhead=8, nhid=512, dropout=0.0, nlayers=3):
        super(Generator_Transformer, self).__init__()
        from torch.nn import TransformerEncoder, TransformerEncoderLayer

        self.linear1 = nn.Linear(1, ntoken) #1,128
        self.pos_encoder = PositionalEncoding(bs, ninp, dropout) # 32,50
    ...
    def forward(self, input): #[bs,50,1] -> [bs,50,1]
        ...
        #- linear [bs,50,1] -> [bs,50, 128]
        li = self.linear1(input)
        
        input_ = li.permute(0,2,1) # bs 128 50
        
        input_ = self.pos_encoder(input_).permute(1,0,2) #128 bs 50
        
        tf = self.transformer_encoder(input_) 
