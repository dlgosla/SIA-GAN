...
class Pre_Transformer(nn.Module):
    def __init__(self, bs, ntoken=128, ninp=50, nhead=5, nhid=512, dropout=0.0, nlayers=1):
        super(Pre_Transformer, self).__init__()
        #from torch.nn import TransformerEncoder , TransformerEncoderLayer
    ...
    def forward(self, input_s, input_f): # [bs, 50, 128], [bs, 50, 128]

        input_s = input_s.permute(2,0,1) # 128 bs 50
        input_f = input_f.permute(2,0,1) 
        #input_sf = input_sf.permute(2,0,1)
        
        tf = self.encoder_layers1(src_Q=input_s, src_K=input_f, src_V=input_f)
