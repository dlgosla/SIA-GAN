...
from network import Pre_Transformer, Multimodal_Transformer, Signal_Encoder, Signal_Decoder,Frequency_1D_Decoder, Frequency_1D_Encoder, Frequency_2D_Encoder, Frequency_2D_Decoder, AD_MODEL, weights_init, print_network
...
class Generator(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.pre_tf = Pre_Transformer(bs=self.bs, ntoken=128, ninp=50, nhead=5, nhid=512, dropout=0.0, nlayers=1)
    ...
    def forward(self, x_sig, x_freq):
        ...
        latent_fusion = self.pre_tf(latent_signal, latent_freq) # pre transformer [bs,50,128]
        
        cls_signal = self.tf(latent_fusion) # multimodal transformer #bs,50,1

        gen_signal = self.signal_decoder(cls_signal) #unimodal decoder #bs,50,128
