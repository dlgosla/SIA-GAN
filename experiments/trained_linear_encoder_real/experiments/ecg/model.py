...
class Generator(nn.Module):
    ...
    def forward(self, x_sig, x_freq):
        ...
        latent_fusion = self.linear4(x_fusion.permute(0,2,1))
...
class BeatGAN(AD_MODEL):
    ...
    def __init__(self, opt, dataloader, device):
        ...
        #    print(name)
        for name, param in self.G.state_dict().items():
            print(name)
        '''
        print(self.G.state_dict()['linear_f.weight'], "original")
        print("====")
        save_dir = os.path.join(self.outf, self.model, self.dataset, "model")
        linear_f = os.path.join(save_dir, "freq_linear.pkl")
        self.G.linear_f.load_state_dict(torch.load(linear_f), strict=False)
        print(self.G.state_dict()["linear_f.weight"], "after")
        '''
        
        #'''
        #original_freq_linear = self.G.state_dict()['linear_f.weight']
        #print(original_freq_linear, "original")
        #save_dir = os.path.join(self.outf, self.model, self.dataset,"model")
        #pretrained_f = torch.load(os.path.join(save_dir,'freq.pkl'))
        #tlqkf = {}
        ...
        tlqkf["linear_f.weight"] = pretrained_f["tf.linear1.weight"]
        tlqkf["linear_f.bias"] = pretrained_f["tf.linear1.bias"]
        torch.save(tlqkf, os.path.join(save_dir, "freq_linear.pkl"))
        print("====")
        ...
        original_signal_linear = self.G.state_dict()['linear4.weight']
        ...
        tlqkf = {}
        tlqkf["linear4.weight"] = pretrained_s["tf.linear1.weight"]
        tlqkf["linear4.bias"] = pretrained_s["tf.linear1.bias"]
        #model_dict = self.G.state_dict()
        #model_dict.update(tlqkf)
        
        self.G.load_state_dict(tlqkf, strict=False)
        #print("========")
        print(self.G.state_dict()["linear4.weight"], "after signal")
        
        
        print(self.G.state_dict()['freq_2d_encoder.main.0.weight'][0][0], "before freq")
        pretrained_freq_encoder = os.path.join(save_dir, 'freq_encoder.pkl')
        self.G.freq_2d_encoder.load_state_dict(torch.load(pretrained_freq_encoder), strict=False)
        print(self.G.state_dict()['freq_2d_encoder.main.0.weight'][0][0], "after freq")
        
        print(self.G.state_dict()['signal_encoder.main.0.weight'][0][0], "before signal")
        pretrained_signal_encoder = os.path.join(save_dir, 'signal_encoder.pkl')
        self.G.signal_encoder.load_state_dict(torch.load(pretrained_signal_encoder), strict=False)
        print(self.G.state_dict()['signal_encoder.main.0.weight'][0][0], "after signal")
        ...
        #save_dir = os.path.join(self.outf, self.model, self.dataset, "model")
        #pretrained_s_linear = os.path.join(save_dir, 'siganl_linear.pkl')
        #self.G.linear_s.load_state_dict(torch.load(pretrained_s_linear), strict=False)
        
        #for key, value in pretrained_s_linear.items():
        #    print(key)
        
        '''
        save_dir = os.path.join(self.outf, self.model, self.dataset, "model")
        original_freq_encoder = self.G.state_dict()['freq_2d_encoder.main.0.weight']
        print(original_freq_encoder[0][0])
        pretrained_freq_encoder = os.path.join(save_dir, 'freq_2d_encoder.pkl')
        self.G.freq_2d_encoder.load_state_dict(torch.load(pretrained_freq_encoder), strict=False)
        #print(self.G.state_dict()['freq_2d_encoder.main.0.weight'][0][0] == original_freq_encoder[0][0] )
        print(self.G.state_dict()['freq_2d_encoder.main.0.weight'][0][0])

        print("------")
        print(self.G.state_dict()['signal_encoder.main.0.weight'][0][0])
        pretrained_signal_encoder = os.path.join(save_dir, 'beatgan_tf_encoder.pkl')
        self.G.signal_encoder.load_state_dict(torch.load(pretrained_signal_encoder), strict=False)
