...
class Multimodal_Transformer(nn.Module):
    ...
    def forward(self, input_s, input_f, input_sf): #[bs,50,128] , [bs,50,128], [bs,50,256]

        input_s = input_s.permute(2,0,1) #128 bs 50
        input_f = input_f.permute(2,0,1) # 128 bs 50
        input_sf = input_sf.permute(2,0,1) # 256 bs 50
        
        tf = self.transformer_encoder(input_sf) #256 bs 50
        ...
        tf = tf.permute(1,2,0) #bs 50 256

        #- parse cls token [bs,50,256] -> [bs,50,1]
...
class AD_MODEL(object):
    ...
    def save_weight_GD_S(self):
        ...
        torch.save(self.G.freq_2d_encoder.state_dict(), os.path.join(save_dir, self.model+"_folder_"+str(self.opt.folder) + '_freq_2d_encoder.pkl'))
        torch.save(self.G.signal_encoder.state_dict(), os.path.join(save_dir, self.model+"_folder_"+str(self.opt.folder) + '_signal_encoder.pkl'))
        torch.save(self.G.signal_decoder.state_dict(), os.path.join(save_dir, self.model+"_folder_"+str(self.opt.folder) + '_decoder.pkl'))
    ...
    def save_weight_GD_F(self):
        ...
        torch.save(self.G.state_dict(), os.path.join(save_dir, self.model+"_folder_"+str(self.opt.folder) + '_G_freq.pkl'))
        torch.save(self.D.state_dict(), os.path.join(save_dir, self.model+"_folder_"+str(self.opt.folder) + '_D_freq.pkl'))
