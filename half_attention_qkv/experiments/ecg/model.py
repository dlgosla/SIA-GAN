...
class Generator(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.opt = opt
        self.tf = Multimodal_Transformer(bs=opt.batchsize, ntoken=256, ninp=50, nhead=5, nhid=512, dropout=0.0, nlayers=3)
    ...
    def forward(self, x_sig, x_freq):
        ...
        return gen_signal1, gen_signal2, latent_signal, latent_freq.squeeze(3)
...
class BeatGAN(AD_MODEL):
    ...
    def update_netd(self):
        ...
        self.fake_s, self.fake_f,_,_ = self.G(self.input_s, self.input_f)
    ...
    def update_netg(self):
        ...
        #print("..?")
        self.fake_s, self.fake_f, self.latent_s, self.latent_f = self.G(self.input_s, self.input_f)
        self.out_d_fake_s, self.feat_fake_s, self.out_d_fake_f, self.feat_fake_f = self.D(self.fake_s, self.fake_f)
        #self.out_d_real_s, self.feat_real_s, self.out_d_real_f, self.feat_real_f = self.D(self.input_s, self.input_f)
        self.out_d_real_s, self.feat_real_s, self.out_d_real_f, self.feat_real_f = self.D(self.input_s, self.input_s)
        ...
        self.err_g = self.err_g_s + self.err_g_f #+ self.err_g_dist
    ...
    def get_generated_x(self, x_type='s'):
        fake_s,fake_f,_,_ = self.G(self.fixed_input_s, self.fixed_input_f)
