...
class Generator_Transformer(nn.Module):
    ...
    def forward(self, input): #input[bs,50,1]
        ...
        #output = self.linear2(tf.permute(1,2,0)) #[bs,50,1]
        output = tf.permute(1,2,0) #[bs,50,128]
       
        cls_token = output[:,:,-1].unsqueeze(2) #[bs,50,1]
...
class AD_MODEL(object):
    ...
    def save_weight_GD(self):
        ...
        torch.save(self.G.encoder1.state_dict(), os.path.join(save_dir, self.model+"_folder_"+str(self.opt.folder) + '_encoder.pkl'))
...
#        ax1.set_ylabel('AUC')     
#        
#        path = os.path.join(self.outf, self.model, self.dataset, model_name + '_auc.png')
#        fig.savefig(path)
