...
class Generator(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.freq_encoder = Frequency_2D_Encoder(opt.ngpu,opt,opt.nz)
    ...
    def forward(self, x_sig, x_freq):
        ...
        latent_signal = latent_signal.squeeze()
        arc_latent_signal = []
        
        for latent in latent_signal:
          label = torch.empty(size=(1, 1), device="cuda:0")
          label.data.fill_(0)
          latent = latent.reshape(1,50)
          #print(cls.shape, cls)
          arc = self.arcface(latent, label)
          arc = arc.reshape(50)
          #print(arc.shape)
          
          arc_latent_signal.append(arc.tolist())
        
        latent_freq = latent_freq.squeeze()
        arc_latent_freq = []
        for latent in latent_freq:
          label = torch.empty(size=(1, 1), device="cuda:0")
          label.data.fill_(1)
          latent = latent.reshape(1,50)
          #print(cls.shape, cls)
          ...
          arc_latent_freq.append(arc.tolist())
        
        arc_latent_signal = torch.Tensor(arc_latent_signal).unsqueeze(2).to("cuda:0")
        arc_latent_freq = torch.Tensor(arc_latent_freq).unsqueeze(2).to("cuda:0")
        
        cls_signal, cls_freq = self.tf(arc_latent_signal, arc_latent_freq) # multimodal transformer
        
        gen_signal1 = self.signal_decoder(cls_signal) #unimodal decoder
        gen_signal2 = self.signal_decoder(cls_freq)
        #gen_signal = (gen_signal1 + gen_signal2) / 2.0
        # gen_freq = self.freq_decoder(cls_freq.unsqueeze(3))

        return gen_signal1, gen_signal2
